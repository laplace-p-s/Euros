...
class Common
{
    ...
    static public function getSummaryInfo(Carbon $now,array $calendar,Carbon $current_now){
        //集計
        $w_time_sum = 0;
        $w_day = 0;
        $w_day_h = 0;
        ...
        //表示用データセット
        $ret = array();
        $ret['now_disp'] = $current_now->format('Y年m月');
        ...
        $last_month = $current_now->copy()->subMonth(1);
        $next_month = $current_now->copy()->addMonth(1);
        ...
        $ret['is_display_next'] = $now->year.$now->month != $current_now->year.$current_now->month;
